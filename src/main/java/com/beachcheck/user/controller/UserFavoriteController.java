package com.beachcheck.user.controller;
...
public class UserFavoriteController {
  ...
  /** 찜 추가 POST /api/favorites/{beachId} */
  @PostMapping("/{beachId}")
  public ResponseEntity<Map<String, Object>> addFavorite(
      @AuthenticationPrincipal User user, @PathVariable UUID beachId) {
    favoriteService.addFavorite(user, beachId);
    return ResponseEntity.ok(Map.of("message", "찜 목록에 추가되었습니다.", "isFavorite", true));
  }

  /** 찜 제거 DELETE /api/favorites/{beachId} */
  @DeleteMapping("/{beachId}")
  public ResponseEntity<Map<String, Object>> removeFavorite(
      @AuthenticationPrincipal User user, @PathVariable UUID beachId) {
    favoriteService.removeFavorite(user, beachId);
    return ResponseEntity.ok(Map.of("message", "찜 목록에서 제거되었습니다.", "isFavorite", false));
  }

  /** 찜 토글 (추가/제거) PUT /api/favorites/{beachId}/toggle */
