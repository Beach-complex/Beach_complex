...
public class ReservationService {
    ...
    /**
     * Why: 예약 생성 규칙을 단일 진입점에서 강제해 일관된 결과를 보장하기 위해.
     * Policy: 예약 시간은 현재 UTC 이후이며 동일 시각 중복 예약은 허용하지 않는다.
     * Contract(Input): userId 또는 beachId가 null이면 예외가 발생한다.
     * Contract(Output): 성공 시 status는 CONFIRMED다.
     */

    public ReservationResponse createReservation(UUID userId, UUID beachId, ReservationCreateRequest request) {
    ...
    /**
     * Why: 예약 삭제 시 소유자와 해변 스코프를 함께 검증해 타 사용자 삭제를 방지하기 위해.
     * Policy: 삭제 대상은 reservationId+userId+beachId 조합으로만 식별한다.
     * Contract:
     * Contract(Input): reservationId와 userId와 beachId로 조회한다.
     *
     * Contract(Output): 해당 조합이 없으면 RESOURCE_NOT_FOUND로 실패한다.
     */
    ...
    /**
     * Why: 이벤트 식별자 입력을 정규화해 의미 없는 값 저장을 방지하기 위해.
     * Policy: 공백만 있는 값은 null로 정규화한다.
     * Contract(Input): eventId가 null일 수 있다.
     * Contract(Output): 반환값은 null 또는 공백이 아닌 문자열이다.
     */
