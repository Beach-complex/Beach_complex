...
public class CacheTestHelper {
  ...
  /**
   * 특정 키의 캐시 값 존재 여부 확인 Why: @CacheEvict 동작 검증 시 사용 Policy: Spring Cache 추상화 사용
   *
   * <p>Note: 이 메서드는 "키가 존재하는지"만 확인합니다. 캐시에 null 값이 저장된 경우에도 true를 반환합니다.
   *
   * @param cacheManager Spring CacheManager
   * @param cacheName 캐시 이름
   * @param key 캐시 키
  ...
  /**
   * 특정 키의 캐시 값 조회 Why: 캐시된 값 검증 시 사용 Policy: Spring Cache 추상화 사용
   *
   * <p>Warning: 이 메서드는 "키가 없는 경우"와 "값이 null인 경우"를 구분하지 않습니다. 두 경우 모두 null을 반환합니다. 키 존재 여부를 확인하려면
   * hasKey()를 먼저 사용하세요.
   *
   * @param cacheManager Spring CacheManager
   * @param cacheName 캐시 이름
  ...
  private static void printCaffeineCacheDetails(CaffeineCache caffeineCache) {
    ...
    if (cacheMap.isEmpty()) {
      ...
      cacheMap.forEach((key, value) -> System.out.println("    키: " + key + " → 값: " + value));
    }
  }
}
