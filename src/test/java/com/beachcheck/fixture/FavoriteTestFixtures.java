...
/**
 * Why: 테스트 데이터 생성 로직 중복 제거 및 재사용성 향상 Policy: 테스트 전용 Fixture이므로 src/test에 위치 Contract(Input): 테스트에서
 * 필요한 도메인 객체 생성 Contract(Output): 일관된 테스트 데이터 제공
 */
public class FavoriteTestFixtures {
  ...
  /**
   * Beach 생성 with Location (통합 테스트용, PostGIS Point 포함) Why: DB 저장 시 location 필드가 NOT NULL이므로 필수
   *
