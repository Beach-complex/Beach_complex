...
이 레포는 Spring Boot 백엔드 애플리케이션의 원본이며, 프론트엔드와 문서는 별도 레포에서 관리합니다.

## 프로젝트 개요

- 프로젝트 성격: 팀 프로젝트
- 목적: 해수욕장 관련 공공데이터와 사용자 기능을 한 서비스 안에서 제공
- 주요 기술: Spring Boot, PostgreSQL(PostGIS), Flyway, Docker Compose
- 현재 구현 중심 흐름: 인증, 해수욕장 조회, 상태 정보 조회, 즐겨찾기, 예약, 알림

## 현재 상태

- 백엔드 API와 배포/테스트 설정이 구성된 저장소입니다.
- 인증, 해수욕장 조회, 즐겨찾기, 예약, 알림 관련 API가 존재합니다.
- DB 마이그레이션은 Flyway로 관리합니다.
- React 프론트엔드는 [Beach-complex/frontend](https://github.com/Beach-complex/frontend), 협업/운영 문서는 [Beach-complex/docs](https://github.com/Beach-complex/docs)에서 관리합니다.
...
```bash
git clone https://github.com/Beach-complex/frontend.git
cd frontend
npm install
npm run dev
```
